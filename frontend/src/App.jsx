...
// ══ BACKGROUND MUSIC (candy-crush style looping melody) ═══════════════════
// All Web Audio — no audio files needed. Plays during matches only.
const MELODY = [
  [784,0.18],[880,0.18],[988,0.18],[1047,0.32],
  [988,0.18],[880,0.18],[784,0.18],[698,0.32],
  [784,0.18],[880,0.18],[784,0.18],[698,0.18],[659,0.32],
  [698,0.18],[784,0.18],[880,0.18],[784,0.32],
  [659,0.18],[698,0.18],[784,0.18],[880,0.32],
  [784,0.18],[698,0.18],[659,0.18],[587,0.32],
  [659,0.18],[698,0.18],[784,0.18],[659,0.32],
  [784,0.46],[0,0.18],
];
const MELODY_LEN = MELODY.reduce((a,[,d])=>a+d, 0);
...
const playMusic = () => {
  ...
  MELODY.forEach(([freq, dur]) => {
    ...
    // Harmony (one octave below, softer)
    const osc2  = ctx.createOscillator();
    const gain2 = ctx.createGain();
    osc2.type = "sine";
    osc2.frequency.value = freq / 2;
    gain2.gain.setValueAtTime(0, t);
    gain2.gain.linearRampToValueAtTime(0.018, t + 0.03);
    gain2.gain.linearRampToValueAtTime(0, t + dur);
    osc2.connect(gain2); gain2.connect(ctx.destination);
    osc2.start(t); osc2.stop(t + dur);
    t += dur;
  });
  _musicTimer = setTimeout(playMusic, MELODY_LEN * 1000);
};
const stopMusic = () => { clearTimeout(_musicTimer); _musicTimer = null; };
const toggleMusic = () => {
  _musicMuted = !_musicMuted;
  if (_musicMuted) stopMusic();
  else playMusic();
  return !_musicMuted; // returns true if now playing
};
const speak = (text) => {
  try {
...
// ══ CANDY DEFINITIONS (CSS-only — no SVG gradient refs) ══════════════════
// Using pure CSS radial-gradient avoids the SVG <defs> isolation bug
// that causes blank game boards in production React builds.
const CANDY = [
  { bg:"radial-gradient(circle at 35% 28%, #FF9999, #FF1133 48%, #7A0000)", glow:"#FF1133", radius:"50%" },
  { bg:"radial-gradient(circle at 35% 28%, #FFD088, #FF8800 48%, #BB3300)", glow:"#FF8800", radius:"50%" },
  { bg:"radial-gradient(circle at 38% 32%, #FFFFAA, #FFDD00 48%, #997700)", glow:"#FFDD00", radius:"50% 50% 50% 50% / 60% 60% 40% 40%" },
  { bg:"radial-gradient(circle at 35% 28%, #AAFFBB, #11BB44 48%, #005520)", glow:"#11BB44", radius:"50%" },
  { bg:"radial-gradient(circle at 33% 26%, #BBDDFF, #1155FF 48%, #000E99)", glow:"#1155FF", radius:"50%" },
  { bg:"radial-gradient(circle at 35% 28%, #EEBCFF, #9911CC 48%, #3A0066)", glow:"#9911CC", radius:"40% 60% 40% 60% / 60% 40% 60% 40%" },
];

// ══ CANDY CELL (pure CSS — always renders correctly) ══════════════════════
const CandyCell = ({ type, selected, bursting, size=44 }) => {
  const def = CANDY[type] || CANDY[0];
  return (
    <div style={{
      ...
      boxShadow: selected
        ? `0 0 0 3px #fff, 0 0 20px ${def.glow}99`
        : `0 4px 10px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.28)`,
      transform: selected ? "scale(1.13)" : "scale(1)",
      transition:"transform 0.1s ease, box-shadow 0.1s ease",
      animation: bursting ? "candyBurst .22s ease-out forwards" : "candyDrop .26s cubic-bezier(0.34,1.56,0.64,1)",
    }}>
      {/* Primary gloss highlight */}
      <div style={{position:"absolute",top:"7%",left:"9%",width:"38%",height:"30%",borderRadius:"50%",background:"rgba(255,255,255,0.62)",filter:"blur(2px)",pointerEvents:"none"}}/>
      {/* Small sparkle */}
      <div style={{position:"absolute",top:"10%",left:"13%",width:"13%",height:"10%",borderRadius:"50%",background:"rgba(255,255,255,0.85)",pointerEvents:"none"}}/>
      {/* Bottom soft reflection */}
      <div style={{position:"absolute",bottom:"7%",left:"22%",right:"22%",height:"16%",borderRadius:"50%",background:"rgba(255,255,255,0.12)",pointerEvents:"none"}}/>
    </div>
  );
};
...
const GamePage = () => {
  const {
    ...
    floatEmojis, emojiCd, swapAnim, handleCell, sendEmoji, nav, musicOn, setMusicOn,
  } = useApp();
  if (!room || !board) return (
    <div style={{minHeight:"100vh",background:"#07071A",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:"20px"}}>
      <div style={{fontSize:"56px",animation:"spin 1s linear infinite"}}>🍬</div>
      <div style={{color:"rgba(255,255,255,0.55)",fontSize:"15px",fontWeight:"600"}}>Setting up the arena...</div>
    </div>
  );
      ...
      <div style={{background:"rgba(7,7,26,0.97)",borderBottom:`1px solid ${V.border}`,padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"10px"}}>
        ...
        <div style={{display:"flex",gap:"6px"}}>
          <button onClick={()=>{
            ...
            setMusicOn(on);
          }} style={{background:"rgba(255,255,255,0.06)",border:`1px solid ${V.border}`,borderRadius:"8px",padding:"8px 12px",cursor:"pointer",fontSize:"16px",color:V.txt,fontFamily:"inherit"}}
          ...
          <div style={{background:"linear-gradient(160deg,#1E0535 0%,#120225 100%)",borderRadius:"22px",padding:"12px",border:`2px solid rgba(150,50,220,0.35)`,boxShadow:"0 0 60px rgba(109,40,217,0.2)",opacity:gameOver?0.65:1,transition:"opacity .4s"}}>
            {board.map((row,r)=>(
              <div key={r} style={{display:"flex",gap:"5px",marginBottom:"5px"}}>
                {row.map((cv,c)=>{
                  // Slide animation: compute CSS transform for the two swapping cells
                  const STEP = 55; // cell (50px) + gap (5px)
                  let slideStyle = {};
                  if (swapAnim) {
                    if (swapAnim.from.r===r && swapAnim.from.c===c) {
                      const dx = (swapAnim.to.c - swapAnim.from.c) * STEP;
                      const dy = (swapAnim.to.r - swapAnim.from.r) * STEP;
                      slideStyle = { transform:`translate(${dx}px,${dy}px)`, transition:"transform 0.18s cubic-bezier(0.4,0,0.2,1)", zIndex:20, position:"relative" };
                    } else if (swapAnim.to.r===r && swapAnim.to.c===c) {
                      const dx = (swapAnim.from.c - swapAnim.to.c) * STEP;
                      const dy = (swapAnim.from.r - swapAnim.to.r) * STEP;
                      slideStyle = { transform:`translate(${dx}px,${dy}px)`, transition:"transform 0.18s cubic-bezier(0.4,0,0.2,1)", zIndex:20, position:"relative" };
                    }
                  }
                  return (
                    <div key={c} style={slideStyle}>
                      <BoardCell onClick={()=>handleCell(r,c)}>
...
// ══ PAGE: PROFILE (name, phone, bank, withdraw) ══════════
const ProfilePage = () => {
  const { user, setUser, token, showToast, bal, witAmt, setWitAmt, doWithdraw, wMsg, doLogout } = useApp();
  ...
  const saveBank = async () => {
    ...
    if (!/^\d{10}$/.test(accNum)) return showToast("Account number must be 10 digits","error");
    setSaving(true);
    const bName = banks.find(b=>b.code===bankCode)?.name||"";
    const d = await apiFetch("PUT","/profile/bank",{ bankName:bName, bankCode, accountNumber:accNum, accountName:accName },token);
    setSaving(false);
  ...
  return (
    <div style={{minHeight:"100vh",paddingTop:"60px",paddingBottom:"90px",background:V.bg}}>
      ...
      <div style={{maxWidth:"600px",margin:"0 auto",padding:"24px 16px"}}>
        ...
        <p style={{color:V.txt2,fontSize:"13px",marginBottom:"22px"}}>Manage your details and bank account</p>

        <Card style={{marginBottom:"16px"}}>
          <div style={{fontWeight:"800",fontSize:"16px",marginBottom:"16px"}}>👤 Personal Information</div>
          <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
            <Field label="FULL NAME" value={name} onChange={setName} placeholder="Your name"/>
            <Field label="PHONE NUMBER" value={phone} onChange={setPhone} placeholder="08012345678" type="tel"/>
            <div style={{fontSize:"12px",color:V.txt3}}>Your phone number is used for account security and withdrawals.</div>
            <Btn onClick={saveProfile} v="out" style={{padding:"12px",fontSize:"14px"}}>Save Profile</Btn>
          </div>
        </Card>

        <Card style={{marginBottom:"16px"}}>
          <div style={{fontWeight:"800",fontSize:"16px",marginBottom:"10px"}}>Email Address</div>
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:"10px",padding:"13px 15px",fontSize:"14px",color:V.txt2,border:`1px solid ${V.border}`}}>{user?.email}</div>
          <div style={{fontSize:"11px",color:V.txt3,marginTop:"8px"}}>Email cannot be changed. Contact support if needed.</div>
        </Card>

        <Card style={{marginBottom:"16px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
            <div style={{fontWeight:"800",fontSize:"16px"}}>🏦 Bank Account</div>
            {hasBankAccount
              ? <span style={{background:"rgba(16,185,129,0.15)",color:V.grn,borderRadius:"7px",padding:"4px 12px",fontSize:"12px",fontWeight:"800"}}>Saved</span>
              : <span style={{background:"rgba(245,158,11,0.15)",color:V.acc,borderRadius:"7px",padding:"4px 12px",fontSize:"12px",fontWeight:"800"}}>Not set</span>}
          </div>
          {hasBankAccount && (
            <div style={{background:"rgba(16,185,129,0.07)",border:`1px solid rgba(16,185,129,0.2)`,borderRadius:"10px",padding:"12px 15px",marginBottom:"14px"}}>
              <div style={{fontSize:"12px",color:V.grn,fontWeight:"700",marginBottom:"4px"}}>CURRENT ACCOUNT</div>
              <div style={{fontWeight:"800",fontSize:"14px"}}>{user.bankAccount.bankName}</div>
              <div style={{color:V.txt2,fontSize:"13px",marginTop:"2px"}}>{user.bankAccount.accountName} · {user.bankAccount.accountNumber}</div>
            </div>
          )}
          <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
            <Select label="SELECT BANK" value={bankCode} onChange={setBankCode} options={banks} placeholder="Choose your bank..."/>
            <Field label="ACCOUNT NUMBER (10 DIGITS)" value={accNum} onChange={v=>setAccNum(v.replace(/\D/g,"").slice(0,10))} placeholder="0123456789"/>
            <Field label="ACCOUNT NAME" value={accName} onChange={setAccName} placeholder="Name on bank account"/>
            <Btn onClick={saveBank} dis={saving} v="gold" style={{padding:"13px",fontSize:"14px"}}>
              {saving?"Saving...":hasBankAccount?"Update Bank Account":"Save Bank Account"}
            </Btn>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div style={{fontSize:"13px",color:V.txt2,marginBottom:"12px"}}>Available: <strong style={{color:V.acc,fontSize:"16px"}}>{fmt(bal)}</strong></div>
          {!hasBankAccount ? (
            <div style={{background:"rgba(245,158,11,0.08)",border:`1px solid rgba(245,158,11,0.25)`,borderRadius:"10px",padding:"14px",fontSize:"13px",color:V.acc}}>
              Add a bank account above to enable withdrawals.
            </div>
          ) : (
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              <Field label="AMOUNT (MIN ₦500)" value={witAmt} onChange={setWitAmt} placeholder="Enter amount" type="number"/>
              <div style={{background:"rgba(255,255,255,0.03)",borderRadius:"9px",padding:"11px 13px",fontSize:"13px"}}>
                <div style={{color:V.txt3,fontSize:"10px",letterSpacing:"1px",marginBottom:"3px"}}>WITHDRAW TO</div>
                <div style={{fontWeight:"700"}}>{user.bankAccount.bankName}</div>
                <div style={{color:V.txt2,fontSize:"12px"}}>{user.bankAccount.accountName} · {user.bankAccount.accountNumber}</div>
              </div>
              <Btn onClick={doWithdraw} v="pri" style={{padding:"13px",fontSize:"15px",fontWeight:"900"}}>🏦 Withdraw Now</Btn>
            </div>
          )}
  ...
  return (
    <div style={{minHeight:"100vh",paddingTop:"60px",paddingBottom:"90px",background:V.bg}}>
      ...
      <div style={{maxWidth:"640px",margin:"0 auto",padding:"24px 16px"}}>
        ...
        {/* Sign Out */}
        <div style={{marginTop:"8px",paddingTop:"16px",borderTop:`1px solid ${V.border}`}}>
          <button onClick={doLogout} style={{
            width:"100%", padding:"16px", border:`1px solid rgba(239,68,68,0.35)`,
            borderRadius:"14px", background:"rgba(239,68,68,0.07)",
            color:"#FCA5A5", fontSize:"15px", fontWeight:"800",
            cursor:"pointer", fontFamily:"inherit",
            display:"flex", alignItems:"center", justifyContent:"center", gap:"10px",
            transition:"all 0.2s",
          }}
          onMouseEnter={e=>{e.currentTarget.style.background="rgba(239,68,68,0.16)";e.currentTarget.style.borderColor="rgba(239,68,68,0.6)";}}
          ...
          >
            <span style={{fontSize:"20px"}}>🚪</span> Sign Out of CrushCash
          </button>
          <p style={{textAlign:"center",fontSize:"11px",color:V.txt3,marginTop:"10px"}}>You can sign back in anytime to continue playing.</p>
        </div>
...
export default function App() {
  ...
  const [swapAnim,  setSwapAnim]  = useState(null); // {from:{r,c}, to:{r,c}}
  ...
  const [musicOn, setMusicOn]   = useState(true);    // background music toggle
  const [navAnim, setNavAnim]   = useState(false);   // page-change candy spin
  ...
  useEffect(()=>{
    ...
    s.on("game_start", d => {
      ...
      setSel(null); setBurstMask(null); setSwapAnim(null); setTLeft(GT); setGameOn(true); setGameOver(false); setGResult(null); setDisqMsg("");
      setRoom(r => r); // keep room
      setTimeout(() => {
        setPg("game");
        sounds.start();
        if (musicOn) { stopMusic(); setTimeout(playMusic, 500); }
  ...
  const nav = useCallback(p=>{
    ...
    // Instant for game-critical navigation (no delay for responsiveness)
    const instant = ["game","matchmaking","auth"].includes(p);
    if (instant) { setPg(p); return; }

    // Page transition: candy spin for user-initiated nav
    setNavAnim(true);
    setTimeout(() => {
      setPg(p);
      setTimeout(() => setNavAnim(false), 120);
    }, 220);
  },[]);
  ...
  const handleCell = (r, c) => {
    // Block input during any animation phase
    if (!gameOn || gameOver || disqMsg || burstMask || swapAnim) return;
    ...
    if (isAdj(sel.r, sel.c, r, c)) {
      ...
      if (res) {
        ...
        // Phase 1: two candies slide into each other's position (180ms CSS transition)
        setSwapAnim({ from, to });

        setTimeout(() => {
          setSwapAnim(null);
          // Phase 2: reveal matched positions with burst highlight (220ms)
          setBoard(res.swapped);
          setBurstMask(res.mask);

          setTimeout(() => {
            // Phase 3: resolve cascades, new candies fall in
            const { b, pts, combo } = resolve(res.swapped);
            sounds.burst(combo);
            setBoard(b);
            setBurstMask(null);
            setScore(prev => { const ns = prev + pts; scoreRef.current = ns; return ns; });
          }, 220);
        }, 180);
      } else {
        sounds.invalid();
  ...
  const ctxValue = {
    ...
    room, board, score, oppScores, sel, burstMask, swapAnim, tLeft, gameOn, gameOver, gResult, sessionId, cdLeft, disqMsg, floatEmojis, emojiCd,
  ...
  return (
    <Ctx.Provider value={ctxValue}>
      <style>{`
        ...
        @keyframes candyDrop{0%{transform:translateY(-20px) scale(0.5);opacity:0}70%{transform:translateY(3px) scale(1.1);opacity:1}100%{transform:translateY(0) scale(1);opacity:1}}
        @keyframes candyBurst{0%{transform:scale(1);opacity:1}50%{transform:scale(1.6);opacity:0.8}100%{transform:scale(0.1);opacity:0}}
        ...
        @keyframes navCandySpin{0%{transform:rotate(0deg) scale(1)}50%{transform:rotate(180deg) scale(1.4)}100%{transform:rotate(360deg) scale(1)}}
        @keyframes pageSlideIn{0%{opacity:0;transform:translateY(12px)}100%{opacity:1;transform:translateY(0)}}
      `}</style>

      {/* Page-change candy spin overlay */}
      {navAnim && (
        <div style={{position:"fixed",inset:0,zIndex:9998,background:"rgba(7,7,26,0.9)",display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none",backdropFilter:"blur(6px)"}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:"64px",animation:"navCandySpin 0.42s cubic-bezier(0.4,0,0.2,1)"}}>🍬</div>
          </div>
        </div>
      )}
      <Toast/>
      <div style={{minHeight:"100vh",background:V.bg,color:"#F1F5F9",fontFamily:"'Segoe UI',system-ui,-apple-system,sans-serif",animation:"pageSlideIn 0.25s ease"}}>
        {pg==="landing"     && <Landing/>}
        {pg==="auth"        && <AuthPage/>}
